...
public class cl_rep_cumplimiento_dia extends Activity{
	...
	private float[] yData;
	private String[] xData={"Cumplido","Pendiente"};
	
	public void onCreate(Bundle savedData) {
		super.onCreate(savedData);
		this.setContentView(R.layout.rep_cumplimiento_dia_ui);
		context = this;
		...
		btnAtras.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				Intent menu = new Intent(cl_rep_cumplimiento_dia.this, cl_consultas.class);
				startActivity(menu);
				finish();
			}
		});
		...
		yData=db.getCumplimientoDelDia();
		
		
		//**********************
		
		rlConsultas=(RelativeLayout)findViewById(R.id.rlCumplimientoDia);
		mChart=new PieChart(this);
		//mChart.setLayoutParams(new LinearLayout.LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT,1.5f));
		...
		mChart.setDescription("Cumplimiento del día");
		
		//conigure x-labels from slice
		...
		mChart.setOnChartValueSelectedListener(new OnChartValueSelectedListener() {
			...
			public void onValueSelected(Entry e, int dataSetIndex, Highlight h) {
					...
					Toast.makeText(cl_rep_cumplimiento_dia.this, xData[e.getXIndex()]+" = "+df.format((e.getVal()*100/mChart.getYValueSum()))+"%", Toast.LENGTH_SHORT).show();
